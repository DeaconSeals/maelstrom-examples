...
@GeneticTree.declare_primitive(PREDATOR, DISTANCE, ())
def distance_to_center_predator(context):
    world: World = context["world"]
    return world.predatorPolar[0]
    # return world.cartesian_to_polar(*world.predator)[0]


@GeneticTree.declare_primitive(PREY, DISTANCE, ())
def distance_to_center_prey(context):
    world: World = context["world"]
    return world.preyPolar[0]
    # return world.cartesian_to_polar(*world.prey)[0]


@GeneticTree.declare_primitive(PREDATOR, ANGLE, ())
...
@GeneticTree.declare_primitive(PREY, ANGLE, ())
...
@GeneticTree.declare_primitive(GENERAL, DISTANCE, ())
...
@GeneticTree.declare_primitive(GENERAL, ANGLE, ())
...
@GeneticTree.declare_primitive(GENERAL, ANGLE, (ANGLE, ANGLE), transitive=True)
...
@GeneticTree.declare_primitive(GENERAL, ANGLE, (ANGLE, DISTANCE), transitive=True)
def multiply_angle(angle, distance):
    return math.fmod(angle * distance, 2 * math.pi)


@GeneticTree.declare_primitive(GENERAL, DISTANCE, (DISTANCE, DISTANCE), transitive=True)
