...
def evaluateMatches(
    ...
    tasks = [
        unorderedWrapper(
            match_id,
            evaluate,
            predators[match[0]].genotype,
            prey[match[1]].genotype,
            world_kwargs,
        )
...
def sampleEvaluations(
    ...
    for predator in range(len(predators.population)):
        ...
        for i in range(samples):
            ...
            opponent = random.choice(
                [
                    i
                    for i in range(len(preyCounter))
                    if preyCounter[i] == minimum and i not in opponents
                ]
            )
    ...
    results = evaluateMatches(
        predators.population, prey.population, matches, executor, world_kwargs
    )
...
def evaluateGenes(
    ...
    tasks = [
        unorderedWrapper(
            match_id, evaluate, predators[match[0]], prey[match[1]], world_kwargs
        )
        for match_id, match in enumerate(matches)
    ]
    chunks_per_processor = 10
    ...
    if progress:
        pbar = tqdm(
            total=len(tasks), unit=" evals"
        )  # with tqdm(total = len(tasks), unit=' evals') as pbar:
...
def postHocRoundRobin(
    ...
    matches = (
        (pred_idx, prey_idx)
        for pred_idx in range(num_predators)
        for prey_idx in range(num_prey)
    )
        ...
        with concurrent.futures.ProcessPoolExecutor(parallel_workers) as executor:
            jobs = [
                executor.submit(
                    ...
                    predators,
                    prey,
                    [match for match in itertools.islice(matches, sequence_length)],
                    world_kwargs=world_kwargs,
                    cores=worker_cores,
                )
                for _ in range(min(parallel_workers * 2, num_jobs))
            ]
            results = {
                "predators": np.zeros((num_predators, num_prey)),
                "prey": np.zeros((num_prey, num_predators)),
            }
            matchIndex = 0
            for job in range(num_jobs):
                ...
                if job + len(jobs) < num_jobs:
                    jobs[job % len(jobs)] = executor.submit(
                        evaluateGenes,
                        predators,
                        prey,
                        [match for match in itertools.islice(matches, sequence_length)],
